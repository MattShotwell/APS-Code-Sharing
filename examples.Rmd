...
The Glasgow Coma Score (GCS) measured closest to 8am was used to evaluate the Brain SOFA-2 component. In APS, the verbal component of GCS is assigned a value of 1 (no verbal response; worst score) for participants who are intubated, and the GCS total score is appended with the character 'L' (e.g., '8L'). However, for the purposes of computing the Brain component of the SOFA-2 score, participants who are intubated are assigned a verbal component of 5 (orientated to person, place, and time; best score). For example, for the purpose of calculating the Brain component of SOFA-2, a GCS '8L' would be treated as 12 = 8 + 4. After accounting for intubated participants, the GCS criteria are applied as described in Table 2 of [Ranzani et al; 2025](https://jamanetwork.com/journals/jama/fullarticle/2840822). If a GCS is reported, that value is used regardless of sedative medication use (this may partially conflict with footnote c). Since in APS the GCS component scores are not recorded individually, it is not possible to impute the GCS from a subset of its component scores (see footnote d). If a participant was receiving drug treatment for delirium and the Brain component is otherwise scored 0, the Brain component is scored 1 instead (consistent with footnote e) If the Brain component is missing (as a result of missing GCS), then the rules for missing values (described below) apply.
...
#### Cardiovascular
Measurements taken closest to 8am were used to evaluate the Cardiovascular SOFA-2 component. MAP was calculated using the following formula: MAP = 1/3 * SBP + 2/3 * DBP.
