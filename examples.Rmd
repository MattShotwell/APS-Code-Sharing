...
# Contributors
  ...
  - Andrew Pulsipher - University of Utah
...
data_sofa <- data %>%
  ...
  select(
    ...
    rrt_stdat,
    rrt_endat,
    ## daily SOFA assessments performed?
    matches('dailysofa_perf_(m2|m1|[0-7])'),
    ## respiratory
  ...
  ## calculate reasons that SOFA-2 scores should be unmeasured
  mutate(sofa_2_unmeasured_reason = case_when(
    
    ## intercurrent event on prior days
    ...
    #dailysofa_perf == 'Not Available' ~ "Marked not available",
    
    ## all SOFA-2 scores missing and intercurrent event same day
    sofa_2_all_na & study_date == as.Date(ds_arrival_date) ~ "Pre-hospital",
    sofa_2_all_na & !is.na(dthdat) & study_date == as.Date(dthdat) ~ "Deceased",
    sofa_2_all_na & !is.na(disdat) & study_date == as.Date(disdat) ~ "Discharged",
    sofa_2_all_na & !is.na(dsstdat) & study_date == as.Date(dsstdat) &
      grepl('Withdrawn|Lost|No further', dsdecod) ~ dsdecod,
    
    ## no clear reason SOFA-2 should be unmeasured
    TRUE ~ NA)) %>%
  
  ## indicate which SOFA scores are eligible for imputation
  mutate(sofa_2_impute_eligible = is.na(sofa_2_totl) &
    is.na(sofa_2_unmeasured_reason)) %>%
  ...
  # readr::write_csv(data_sofa,
  #   file=paste0(timestamp, '_APS_SOFA_2_data.csv'))
  # 
  # data_icu_death <- data %>%
  #   filter(!duplicated(record_id)) %>%
  #   select(record_id) %>%
  #   ## day 0 enrollment date
  #   ## hospital discharge ventilator data
  #   left_join(result %>%
  #     filter(grepl('Hospital Discharge', redcap_event_name)) %>%
  #     filter(is.na(redcap_repeat_instrument)) %>%
  #     select(record_id, icu_stdat, icu_discat, icu_endat),
  #     by='record_id') %>%
  #   left_join(result %>%
  #     filter(grepl('Logs', redcap_event_name)) %>%
  #     filter(is.na(redcap_repeat_instrument)) %>%
  #     select(record_id, dsdecod, dsstdat, dthind, dthdat),
  #     by='record_id')
  # 
  # readr::write_csv(data_icu_death,
  #   file=paste0(timestamp, '_APS_ICU_death_data.csv'))
...
data_sofa %>%
  ...
  scale_fill_brewer(palette = "Accent", na.value='grey50',
    labels = function(z) ifelse(is.na(z), "Unmeasured", z))
```

### Plot of total scores

Caption: SOFA-2 total scores from study day -2 through day 7 for the first `r nrow(data_sofa)` participants enrolled in the APS Consortium. During hospitalization, missing organ system scores were imputed using the methods recommended in [Ranzani et al; 2025](https://jamanetwork.com/journals/jama/fullarticle/2840822)(Table 2, footnote b). For all but the Liver component, fewer than 6.5% of participant-days during hospitalization required imputation. For the Liver component, 41% of values during hospitalization were imputed due to unmeasured Bilirubin.

```{r fold=TRUE}
data_sofa_plot <- data_sofa %>%
  select(record_id, study_day, sofa_2_totl_impu, sofa_2_unmeasured_reason) %>%
  mutate(study_day = factor(study_day, levels=-2:7)) %>%
  mutate(sofa_2_totl_impu = cut(sofa_2_totl_impu, breaks=c(-Inf, 2, 5, 8, 11, Inf), 
    labels=c('SOFA-2 score 0-2', 'SOFA-2 score 3-5', 'SOFA-2 score 6-8',
             'SOFA-2 score 9-11', 'SOFA-2 score ≥12'))) %>%
  mutate(sofa_2_totl_impu = case_when(
    !is.na(sofa_2_totl_impu) ~ sofa_2_totl_impu,
    sofa_2_unmeasured_reason %in% c(
      'Pre-hospital', 
      'Discharged', 
      'Discontinued - No further data collection') ~ 'Not in hospital',
    sofa_2_unmeasured_reason == 'Deceased' ~ 'Dead',
    TRUE ~ sofa_2_unmeasured_reason
  )) %>%
  mutate(sofa_2_totl_impu = factor(sofa_2_totl_impu,
    levels=c('Not in hospital', 'SOFA-2 score 0-2', 'SOFA-2 score 3-5', 'SOFA-2 score 6-8', 'SOFA-2 score 9-11', 'SOFA-2 score ≥12', 'Dead'))) %>%
...
data_sofa_plot %>%
  ggplot(aes(x=study_day, y=count, fill=sofa_2_totl_impu)) +
    geom_bar(stat="identity", position='stack') +
    labs(x='Study Day', y='Participant Count', fill='Key', title='SOFA-2 Total Score') +
  scale_fill_brewer(palette = "Accent", na.value='grey50',
    labels = function(z) ifelse(is.na(z), "Unmeasured", z))
```

### Plot of reasons for unmeasured scores

```{r fold=TRUE}
data_sofa %>%
  mutate(study_day = factor(study_day, levels=-2:7)) %>%
  mutate(sofa_2_reason = case_when(
    !is.na(sofa_2_totl) | !is.na(sofa_2_totl_impu) ~ 'Measured/Imputed',
    # !is.na(sofa_2_totl) ~ 'Measured',
    # is.na(sofa_2_totl) & !is.na(sofa_2_totl_impu) ~ 'Imputed',
    is.na(sofa_2_totl_impu) ~ sofa_2_unmeasured_reason,
    TRUE ~ NA)) %>%
  mutate(sofa_2_reason = factor(sofa_2_reason, levels=c(
    "Pre-hospital", "Discharged", "Deceased",
    "Discontinued - No further data collection",
    "Measured/Imputed"), labels=c(
    "Pre-hospital", "Discharged", "Deceased",
    "Discontinued", "Measured/Imputed"))) %>%
  group_by(study_day, sofa_2_reason) %>%
  summarise(count = n(), .groups='drop') %>%
  ggplot(aes(x=study_day, y=count, fill=sofa_2_reason)) +
    geom_bar(stat="identity", position='stack') +
    labs(x='Study Day', y='Participant Count', fill='Reason', title='SOFA-2 Unmeasured Reason') +
    theme_light() +
  scale_fill_brewer(palette = "Accent", na.value='grey50',
    labels = function(z) ifelse(is.na(z), "Unmeasured", z))
```

### Plot of measured/imputed/unmeasured scores

The table below that shows the percent of imputed versus measured values across all participant-days in the hospital by organ system score. Note that the 'Not in hospital/Dead' values are not all the same across organ systems. This is because on the day of discharge or death, if the data needed to compute the SOFA-2 scores are available, those values are used and the score is reported even though the participant was discharged or died that day. The same is true for the total score. If all component scores are available on the day of death or discharge, the total score is reported for that day. Renal is an outlier in this regard because new or chronic RRT yields a score of 4, and generally this information is available on the day of death or discharge.

```{r}
data_sofa_meas <- data_sofa %>% 
  select(record_id, study_day, matches('sofa_2_(resp|coag|livr|card|cns|rena)$')) %>%
  pivot_longer(
    cols=starts_with('sofa_2'),
    names_to='component',
    names_pattern='^sofa_2_(.+)$',
    values_to = 'value_meas')

data_sofa_impu <- data_sofa %>% 
  select(record_id, study_day, matches('sofa_2_(resp|coag|livr|card|cns|rena)_impu')) %>%
  pivot_longer(
    cols=starts_with('sofa_2'),
    names_to='component',
    names_pattern='^sofa_2_(.+)_impu$',
    values_to = 'value_impu')

data_sofa_meas_impu <- full_join(data_sofa_meas, data_sofa_impu,
  by=c('record_id', 'study_day', 'component')) %>%
  mutate(reason = case_when(
    !is.na(value_meas) ~ 'Measured',
    !is.na(value_impu) ~ 'Imputed',
    TRUE ~ 'Unmeasured'
  ))
...
data_sofa_meas_impu %>%
  ...
  mutate(reason = factor(reason, levels=c('Imputed','Measured'))) %>%
  tbl_summary(
    by='component',
    missing='always',
    missing_text='Not in hospital/Dead', 
    label = list('reason' ~ 'Reason'))
...
data_sofa_meas_impu %>%
  mutate(component = case_match(component,
    ...
    "cns"  ~ "CNS",
    "rena" ~ "Renal")) %>%
  group_by(study_day, component, reason) %>%
  summarise(count = n(), .groups='drop') %>%
  ggplot(aes(x=study_day, y=count, fill=reason)) +
    geom_bar(stat="identity", position='stack') +
    facet_wrap(~component) +
  labs(x='Study Day', y='Participant Count', fill='Score', title='SOFA-2 Component Measured') +
  scale_fill_brewer(palette = "Accent", na.value='grey50',
    labels = function(z) ifelse(is.na(z), "Unmeasured", z))
```
