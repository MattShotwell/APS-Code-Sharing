...
# Calculate STREAMLINED DAG 'Hypotension Severity'
#
# Values:
...
# - 5 = NEE >= 0.25
calc_str_hypotension_sev_0 <- function(
  ...
  dplyr::case_when(
    ...
    # Score 4: 0.1 < NEE < 0.25
    norepi_equiv > 0.1 & norepi_equiv < 0.25 ~ 4,
...
# Returns a data frame with record_id and str_hypotension_sev_0 columns (one row per record_id)
wrapper_calc_str_hypotension_sev_0 <- function(data) {
  data_with_hypotension_sev <- data |>
    ...
    left_join(data |>
      ...
      by = 'record_id') |>
    mutate(
      str_hypotension_sev_0 = calc_str_hypotension_sev_0(
        daily_vasopressors_0___0,
        daily_vasopressors_0___1,
        daily_vasopressors_0___2,
        daily_vasopressors_0___3,
    ...
    select(record_id, str_hypotension_sev_0)
